...
namespace Darwin.Net.Requests
{
    public partial class Requests : IRequests
    {
        public Task<StationBoard> GetDepartureBoardAsync(int numRows, Station station, TimeSpan timeWindow, Station? filterStation = null, FilterType? filterType = null, TimeSpan? timeOffset = null)
        {
            ...
            return GetDepartureBoardInternal(requestParams);
        }

        private async Task<StationBoard> GetDepartureBoardInternal(Dictionary<string, object> requestParams)
        {
            var soapEnvelope = BuildDarwinSoapEnvelope("GetDepartureBoard", requestParams);
