...
namespace Darwin.Net.Requests
{
    public partial class Requests : IRequests
    {
        public Task<StationBoard> GetArrivalBoardAsync(int numRows, Station station, TimeSpan timeWindow, Station? filterStation = null, FilterType? filterType = null, TimeSpan? timeOffset = null)
        {
            ...
            return GetArrivalBoardInternal(requestParams);
        }

        private async Task<StationBoard> GetArrivalBoardInternal(Dictionary<string, object> requestParams)
        {
            var soapEnvelope = BuildDarwinSoapEnvelope("GetArrivalBoard", requestParams);
