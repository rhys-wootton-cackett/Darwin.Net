...
namespace Darwin.Net.Requests
{
    public partial class Requests : IRequests
    {
        public Task<StationBoard> GetArrivalDepartureBoardAsync(int numRows, Station station, TimeSpan timeWindow, Station? filterStation = null, FilterType? filterType = null, TimeSpan? timeOffset = null)
        {
            ...
            if (numRows <= 0 || numRows >= 150) throw new ArgumentOutOfRangeException(nameof(numRows), "numRows has to be between 0 and 150 exclusive.");
            ...
            return GetArrivalDepartureBoardInternal(requestParams);
        }

        private async Task<StationBoard> GetArrivalDepartureBoardInternal(Dictionary<string, object> requestParams)
        {
            var soapEnvelope = BuildDarwinSoapEnvelope("GetArrivalDepartureBoard", requestParams);
            var response = await SendDarwinSoapRequestAsync(soapEnvelope, StationBoardFactory.Instance);
            return response;
        }
    }
}
